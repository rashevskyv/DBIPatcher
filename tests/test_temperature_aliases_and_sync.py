...
class TemperatureAliasesAndSyncTests(unittest.TestCase):
    ...
    def test_workbook_structure_and_version(self) -> None:
        """Verify workbook contains 1,430 unique Original keys, 'tr' and 'id' columns, and version 0.0.104."""
        ...
        self.assertEqual(version, "0.0.104", f"Expected version 0.0.104, got {version}")
        ...
        self.assertEqual(
            len(originals),
            1430,
            f"Expected 1,430 original keys, found {len(originals)}",
        )
        self.assertEqual(
            len(set(originals)),
            1430,
            f"Expected 1,430 unique original keys, found {len(set(originals))}",
        )
    ...
    def test_exported_csv_keys_match_workbook(self) -> None:
        ...
        self.assertEqual(len(wb_tokenized_keys), 1430)
